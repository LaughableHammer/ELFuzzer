...
// Copied from https://github.com/rgriege/amethyst/blob/master/example.c
// A format string vulnerability has been added on line 113
...
int page_draw(struct pdf *pdf, int page_idx)
{
	...
	if (resources) {
		...
		for (size_t i = 0; i < resources->dict.sz; ++i) {
			printf(resources->dict.entries[i].name);
...
int main(int argc, const char *argv[])
{
	struct pdf pdf = {0};
	int ret = 1, pages;
	if (!PDF_OK(pdf_init_from_file(&pdf, "/proc/self/fd/0")))
		goto out;
